...
public class MashableTest {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < oa.length; i++) {
            int[] iterations = {2000,4000,6000};
            System.out.println("\nError results for " + oaNames[i] + "\n");
            for (int k = 0; k < iterations.length; k++ ) {
	            double start = System.nanoTime(), end, trainingTime, testingTime, correct = 0, incorrect = 0;
	            train(oa[i], networks[i], oaNames[i], iterations[k]); //trainer.train();
	            end = System.nanoTime();
...
//	                        + " seconds\nTesting time: " + df.format(testingTime) + " seconds\n";
	            ...
	            String result = "\n " + iterations[k] + "," + df.format(correct/(correct+incorrect)*100) ;
	            try {
    ...
    private static void train(OptimizationAlgorithm oa, BackPropagationNetwork network, String oaName, int iterations) {
    	...
    	for(int i = 0; i < iterations; i++) {
	        ...
	        String result = iterations + "," + df.format(correct/(correct+incorrect)*100) ;
            try {
